        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="bg-midnight-light border border-white/10 rounded-[4rem] w-full max-w-5xl overflow-hidden shadow-2xl relative flex flex-col lg:flex-row h-[700px]"
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Close Button */}
                    <button onClick={onClose} className="absolute top-8 right-8 z-50 p-4 glass rounded-full text-ivory/50 hover:text-champagne transition-colors">
                        <X size={24} />
                    </button>

                    {/* Left Sidebar */}
                    <div className="w-full lg:w-80 border-r border-white/5 p-12 lg:p-16 flex flex-col gap-8 bg-black/20">
                        <div className="mb-8">
                            <span className="text-champagne font-bold text-[10px] tracking-[0.5em] uppercase mb-4 block">Standards</span>
                            <h2 className="font-serif text-3xl font-bold text-ivory">Institutional <br /><span className="text-champagne italic">Policies</span></h2>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                            {POLICIES.map((policy) => (
                                <button
                                    ...
                                    className={`flex items-center gap-6 px-8 py-5 rounded-[2rem] text-xs font-bold tracking-widest uppercase transition-all text-left ${activeTab === policy.id ? 'bg-champagne text-midnight shadow-[0_10px_30px_rgba(212,175,55,0.2)]' : 'text-ivory/40 hover:text-ivory hover:bg-white/5'}`}
                                >
                                    <policy.icon size={20} />
                                    {policy.title}
                                </button>
                    ...
                    <div className="flex-1 p-12 lg:p-24 overflow-y-auto">
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                className="max-w-2xl"
                            >
                                <div className="w-20 h-20 bg-champagne/10 rounded-3xl flex items-center justify-center text-champagne mb-12 border border-champagne/20">
                                    <activePolicy.icon size={40} />
                                </div>
                                <h3 className="font-serif text-5xl font-bold text-ivory mb-10 leading-tight">{activePolicy.title}</h3>
                                <div className="space-y-8">
                                    {activePolicy.content.split('\n').map((line, idx) => {
                                        ...
                                        return (
                                            <p key={idx} className={`text-lg leading-relaxed ${isBullet ? 'text-ivory/60 pl-8 relative before:content-[""] before:absolute before:left-0 before:top-4 before:w-2 before:h-2 before:bg-champagne before:rounded-full' : 'text-ivory font-serif italic border-b border-white/5 pb-4'}`}>
